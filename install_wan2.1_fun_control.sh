...
clone_or_update () {
  ...
  if [ -d "$dir_name/.git" ]; then
    ...
    git -C "$dir_name" pull --rebase --autostash || true
  else
...
# Some nodes use extras; we don't force pip here because environments vary.
# If you need them: pip install -r requirements.txt inside each folder.
...
dl "$KJ_WAN14B_FP8_URL" "$KJ_WAN14B_FP8_OUT" "${COMFY}/models/diffusion_models" || true
...
echo
echo "== Done. Models installed in: ${COMFY}/models"
echo
echo "== Tree (models only):"
( cd "${COMFY}/models" && find . -maxdepth 2 -type f | sort )

echo
echo "Tip: Restart ComfyUI. In native flow, use:"
